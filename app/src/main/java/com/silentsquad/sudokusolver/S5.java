...
public class S5 extends AppCompatActivity implements View.OnClickListener{
    ...
    protected void onPause() {
        ...
        text = silentsquad_solved + "-" + silentsquad_unsolved + "-" ;
        for(int i=0 ; i<81 ; i++){
    ...
    static String silentsquad_solved;
    static String silentsquad_unsolved;
        ...
        for(int i=0;i<81;i++){
            if(solving[i/9][i%9]==solved[i/9][i%9]){
                if(i==80){
                    ...
                    Intent winningSplash = new Intent(getApplicationContext(),com.silentsquad.sudokusolver.FinishSplash.class) ;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(getIntent().getBooleanExtra("fromS4beta",true)) {
            resume = true;
            silentsquad_unsolved = getIntent().getStringExtra("unsolved");
            silentsquad_solved = getIntent().getStringExtra("solved");
        }
        else {
            loaddata();
            String[] part = text.split("-");
            silentsquad_solved = part[0];
            silentsquad_unsolved = part[1];
            current = part[2];
            string_timer = part[3];
        ...
        for(int i=0 ; i<81 ; i++){
            int a = silentsquad_unsolved.charAt(i) - '0';
            unsolved[i/9][i%9] =  a;
            solving[i/9][i%9]=a;
        }
        for(int i=0 ; i<81 ; i++){
            int a = silentsquad_solved.charAt(i) - '0';
            solved[i/9][i%9] =  a;
        }
